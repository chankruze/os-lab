...
Created: Wed May 24 2023 21:57:35 GMT+0530 (India Standard Time)
...
void srt(Process processes[], int n) {
  ...
  while (completed != n) {
    ...
    for (int i = 0; i < n; i++) {
      if (processes[i].arrival_time <= total_time && !processes[i].is_completed && processes[i].remaining_time < shortest_burst_time) {
        shortest_job = i;
    ...
    if (processes[shortest_job].remaining_time == 0) {
      processes[shortest_job].waiting_time = total_time - processes[shortest_job].arrival_time - processes[shortest_job].burst_time;
